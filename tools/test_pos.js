...
var cases = [
  ...
  ["N-COUNT a name",
   "a name"],
  ["PHR-MODAL verb used to",
   "used to"],
  ["vt to do something",
   "to do something"],
  ["adv quickly\nadj slow",
   "quickly\nslow"],
  ["plain text without POS",
   "plain text without POS"],
  ["modal verb can",
   "can"],
  ["linking verb 位于；在（某处）\n动词 出生于",
   "位于；在（某处）\n出生于"],
  ["n-COUNT\nverb",
   ""],
  ["VERB transitive",
...
cases.forEach(function (c, i) {
  var got = stripPosLines(c[0]);
  var ok = got === c[1];
  if (ok) pass++; else fail++;
  console.log((ok ? "PASS " : "FAIL ") + "#" + (i+1) + " got=" + JSON.stringify(got) +
    (ok ? "" : " want=" + JSON.stringify(c[1])));
});
console.log("---");
